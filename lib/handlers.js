...
handlers.users = async data => {
  ...
  if (acceptableMethods.indexOf(data.method) > -1) {
    ...
    return Promise.reject({
      statusCode: 405,
      message: "Invalid HTTP Method. Method Not Allowed."
    });
  }
};
...
handlers._users.put = async data => {
  try {
    ...
    if (!(phone && (firstName || lastName || password || email))) {
      return Promise.resolve({
        statusCode: 400,
        message: "Missing Fields to Update / Wrong Input Data."
      });
    }
...
*/
handlers._hobby.delete = async clientData => {
  try {
    const phone =
      typeof clientData.queryStringObject.phone == "string" &&
      clientData.queryStringObject.phone.trim().length === 10
    ...
    if (!userData) {
      return Promise.resolve({
        statusCode: 400,
        message: "The Specified User doens not exist"
      });
    }
    ...
    if (!(index > -1)) {
      return Promise.resolve({
        statusCode: 400,
        message: "Hobby Not Found To Delete"
      });
    }
    userData.hobby.splice(index, 1);

    await _data.update("users", phone, userData);
    return Promise.resolve({ statusCode: 200, message: "User Hobby Deleted" });
  } catch (err) {
    console.error(err);
    return Promise.reject({ statusCode: 500, message: "Server Error" });
  }
...
handlers.age = data => {
  ...
  if (acceptableMethods.indexOf(data.method) > -1) {
    return handlers._age[data.method](data);
  } else {
    Promise.reject({
      statusCode: 405,
      message: "Invalid HTTP Method. Method Not Allowed."
...
handlers._age.get = async data => {
  try {
    ...
    const age = days > 0 ? `${days} days.` : `${hours} hours.`;

    return Promise.resolve({ statusCode: 200, message: { Age: age } });
  } catch (error) {
    console.error(error);
    return Promise.reject({ statusCode: 500, message: "Server Error" });
  }
