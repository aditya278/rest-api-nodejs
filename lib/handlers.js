...
handlers.users = (data) => {
  ...
  if (acceptableMethods.indexOf(data.method) > -1) {
    ...
    return ({'statusCode': 405, "message": "Invalid HTTP Method. Request Failed." });
  }
};
handlers._users = {};
...
handlers._users.put = async (data) => {
  try {
    ...
    if (!(phone && (firstName || lastName || password || email))) {
      return Promise.resolve({'statusCode': 400, 'message':'Missing Fields to Update'});
    }
    ...
    await _data._update('users', phone, userData);
